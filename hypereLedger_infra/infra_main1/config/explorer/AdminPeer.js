...
var protoPath=appRoot + '/node_modules/fabric-client/lib/protos/peer/admin.proto'
//  protoLoader.load(protoPath, options).then(packageDefinition => {
//     const _serviceProto = grpcLibrary.loadPackageDefinition(packageDefinition);
//   });
...
class AdminPeer {
  ...
  constructor(mspid, peer, roles) {
    ...
    if (peer && peer.constructor && peer.constructor.name === 'Peer') {
      ...
      this._peer = peer;
      this._roles = {};
      console.log('AdminPeer.const - url: %s', peer.getUrl());
