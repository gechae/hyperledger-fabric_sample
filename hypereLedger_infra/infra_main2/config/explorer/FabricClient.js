...
const protoPath=appRoot + "/node_modules/fabric-client/lib/protos/common/common.proto";
  const packageDefinition = protoLoader.loadSync(protoPath);
  const _commonProto = grpc.loadPackageDefinition(packageDefinition).common;

  const _commonProto_Paload = grpc.loadPackageDefinition(packageDefinition).common.Payload
...
class FabricClient {
				...
				try {
					if (peer_config && peer_config.tlsCACerts) {
                        ...
                        let adminpeer = await this.newAdminPeer(
                          ...
                          console.log("out newAdminPeer~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                          console.log("adminpeer")
                          if (adminpeer) {
		...
		if (discover_results) {
			if (discover_results.msps) {
				for (const msp_name in discover_results.msps) {
                    ...
                    console.log("set admin uaser????~~~~~~~~~~~")
                    let username = this.client_name + '_' + msp.id + 'Admin';
                    console.log(username)
                    if (!this.adminusers.get(username)) {
                        console.log("set admin user if if if~~~~~~~~~~~")
      ...
      if (discover_results && discover_results.peers_by_org) {
        for (let org_name in discover_results.peers_by_org) {
          ...
          for (var peer of org.peers) {
            console.log("########pre new admin peer########");
            console.log(peer);
            let host = peer.endpoint.split(':')[0];
 
            if (
              this.config.peers &&
              this.config.peers[host] &&
              this.config.peers[host].url
            ) {
	...
	newPeer(channel, url, msp_id, host, msps) {
        ...
        console.log("########2 new peer########");
        console.log("########2 new peer########");
        console.log("########2 new peer########");
        
        console.log("########2 new peer########");
		if (!newpeer) {
            console.log("########2if new peer########");
            console.log("########2if new peer########");
            console.log("########2if new peer########");
			if (msps[msp_id]) {

                console.log("########3if new peer########");
                console.log("########3if new peer########");
                console.log("########3if new peer########");
	...
	async newUser(msp_name, username, msp_admin_cert) {
		// const organization = await this.hfc_client._network_config._network_config.getOrganization(
		// 	msp_name,
		// 	true
        // );
        const organization = this.hfc_client._network_config._network_config.organizations
        console.log("################organization############")
        console.log(organization) 
        // console.log(this.hfc_client._network_config._network_config.organizations) 
        // console.log(organization) 
		if (!organization) {
        ...
        const mspid = organization.nonghyupitMSP.mspid;
        ...
        const admin_key = organization.nonghyupitMSP.adminPrivateKey;
		let admin_cert = organization.nonghyupitMSP.signedCert;
		if (!admin_cert) {
    ...
    async getPeerStatus(peer) {
        ...
        console.log("###########################################")
        console.log(peer)
        console.log(this.adminpeers)
        console.log("###########################################")
        console.log("###########################################")
        console.log("###########################################")
        let channel = this.getDefaultChannel();
        // let adminpeer = this.adminpeers.get(peer.requests);
        let adminpeer = this.adminpeers.get("grpcs://peer2.orgnonghyupit.com:7051");
        // grpcs://peer2.orgnonghyupit.com:7051
        console.log("##################adminpeer#################")
        console.log("###########################################")
        console.log("###########################################")
        console.log(adminpeer)
        ...
        if (adminpeer) {
          ...
          if (user) {
            ...
            console.log("#######################_commonProto.HeaderType.PEER_ADMIN_OPERATION####################")
            // build the header for use with the seekInfo payload
           console.log(_commonProto.HeaderType.PEER_ADMIN_OPERATION)
           console.log("#####################channel._name######################")
           console.log(channel._name)
           console.log("########################txId.getTransactionID()###################")
            console.log(txId.getTransactionID())
            console.log("#######################channel._initial_epoch####################")
            console.log(channel._initial_epoch)
            console.log("#########################lient_utils.buildCurrentTimestamp()##################")
            console.log(client_utils.buildCurrentTimestamp())
            console.log("######################channel._clientContext.getClientCertHash()#####################")
            console.log(channel._clientContext.getClientCertHash())
            console.log("###########################################")
